...
import views.Main;
	...
	public class Game {
		...
		/**
		 * Updates visibilty of all cells in map
		 */
		public static void updateMap() {
			...
			for(int i=0; i<15; i++) {
				for(int j=0; j<15; j++) {
					...
					if(map[i][j] instanceof CharacterCell) {
						((CharacterCell) map[i][j]).setSafe(true);
					}
				}
			}
			...
			zombies.forEach((zombie)-> {
				...
				for(int j=x-1; j<x+2; j++) {
					for(int k=y-1; k<y+2; k++) {
						if(j >= 0 && j < 15 && k >= 0 && k < 15) {
							if(map[j][k] instanceof CharacterCell) {
								((CharacterCell) map[j][k]).setSafe(false);
							}
						}
					}
				}
		 ...
		 * @param h Hero to spawn at start of game
		 */
		...
		/**
		 * Ends turn of game, making zombies attack, spawning a zombie, and reseting stats and making Ai play if needed
		 */
		public static void endTurn() throws MovementException, NotEnoughActionsException, NoAvailableResourcesException, InvalidTargetException {
		...
		/**
		 * spawns given number of zombies if doesnt exceed 10 on map
		 * @param numberOfZombies number of zombies to spawn
		 */
